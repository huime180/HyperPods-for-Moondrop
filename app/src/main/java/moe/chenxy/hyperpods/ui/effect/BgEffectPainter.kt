...
// 注意（与参考实现的唯一差异）：miuix 0.9.3 把 RuntimeShader 从 blur 包挪到了 shader 包，
// blur 包里只留下一层兼容壳（同名函数 + 同名扩展）。这里直接用 0.9.3 的真实包路径，
// 已对着 miuix-shader-android:0.9.3 的 classes.jar 核对过：
//   top.yukonga.miuix.kmp.shader.RuntimeShader（类，含 setFloatUniform(String, Float/Float[])）
//   top.yukonga.miuix.kmp.shader.RuntimeShader(String)（工厂函数）
//   top.yukonga.miuix.kmp.shader.asBrush()（扩展，返回 ShaderBrush）
