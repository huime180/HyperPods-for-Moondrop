 ...
 *     · 本接收器 —— 监听**系统蓝牙进程**在 A2DP 连接状态变化时发出的系统广播（manifest 静态注册，
 *       见 AndroidManifest.xml），于是「从未打开过 App + 在系统蓝牙里连上耳机」时进程也能被唤醒，
 *       pods/PodNotification.kt 才有机会发出那条通知。
 *   协议栈本来就在应用进程（pods/MoondropLink.kt 自己 connectGatt / 建 RFCOMM），整条路不需要 hook：
 *   不新增连接路径、不重复实现电量解析、不持有唤醒锁、不起常驻服务。
 *
 * 判据与取舍：
 *   · 只在 A2DP STATE_CONNECTED 上动作；其它状态 / 缺 device extra 一律静默返回；
 *   · 「是不是水月雨」优先用设备名（[MoondropModels.isMoondrop] 的别名匹配），
 *     名字拿不到（缺 BLUETOOTH_CONNECT / 名字为空）才回落到 App 已存的「上次连接地址」
...
import moe.huime.miuixmoondrop.ui.UI_PREFS_GROUP
import moe.huime.miuixmoondrop.ui.UI_PREFS_KEY_LAST_ADDRESS
...
class BluetoothConnectReceiver : BroadcastReceiver() {
    ...
    /**
     * 只在「A2DP 连上 + 是水月雨设备」时做一件事：用应用侧入口连一次耳机。
     * 其余情况（其它状态、没有 device extra、不是水月雨）一律静默返回，无副作用。
     */
    private fun handle(context: Context, intent: Intent) {
        ...
        // 应用侧入口初始化：appContext + 状态转发器（与 ui/PodState.kt 的冷启动兜底同一条路；
        // 不发它的话 MoondropLink 没有 appContext，connect 会直接返回）
        ControlBridge.ensureInit(context.applicationContext)
        Log.i(TAG, "A2DP connected -> app-side connect")
     ...
     *    读名字需要 BLUETOOTH_CONNECT，没授权会抛 SecurityException —— 这里按「名字拿不到」处理，
     *    而不是当成「不是水月雨」。
